...
class EKF {
public:
  EKF(Eigen::MatrixXd mu, Eigen::MatrixXd sigma, std::vector<sensors> sens, std::vector<int> obser)
      : mu_(mu), sigma_(sigma), sensdata_(sens), observed_(obser) {
    dim = sigma.cols();
  ...
  void param_init(Eigen::MatrixXd mu, Eigen::MatrixXd sigma, std::vector<sensors> sens,
                  std::vector<int> obser);
