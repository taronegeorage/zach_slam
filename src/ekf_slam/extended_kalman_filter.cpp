...
void EKF::correction_step(int step) {
  ...
  for (int i = 0; i < mNum; ++i) {
    ...
    // TODO: Use the current estimate of the landmark pose to compute the corresponding expected
    // measurement in expectedZ:
    ...
    Hi << -sqrt(distance2) * deltax, -sqrt(distance2) * deltay, 0, sqrt(distance2) * deltax,
        sqrt(distance2) * deltay, deltay, -deltax, -distance2, -deltay, deltax;
    Hi = 1 / distance2 * Hi;
...
void EKF::plot_state(int step) {
  ...
  cv::circle(out, cv::Point(center + mu_(0), center + mu_(1)), 4, cv::Scalar(0, 0, 255), -1, 8);
  cv::imshow("traj", out);
  cv::waitKey(0);
}
